...
private:
    ...
    void left_rotate(Node<K, V> *x) {
		...
		if( x == x->parent()->left()) { x->parent()->set_left(y); }
    ...
    void right_rotate(Node<K, V> *x) {
    	...
    	y->set_parent(x->parent());
    	if(x == x->parent()->left()) { x->parent()->set_right(y); }
    	else { x->parent()->set_right(y); }
    	y->set_right(x);
    	x->set_parent(y);
    }
    ...
    size_t leaf_count(Node<K, V> *node) const {
		...
		if(node->left() == NULL && node->right() != NULL) { return leaf_count(node->right()); }
		if(node->right() == NULL && node->left() != NULL) { return leaf_count(node->left()); }
